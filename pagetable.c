 ...
 */
 int allocate_frame(pgtbl_entry_t *p) {
	...
	coremap[frame].pte = p;
	coremap[frame].distance = p->distance;
...
char *find_physpage(addr_t vaddr, char type) {
	...
	unsigned idx = PGDIR_INDEX(vaddr); // get index into page director
	ref_count++;
	// IMPLEMENTATION NEEDED
    ...
    uintptr_t table_ptr = PAGE_MASK & pgdir[idx].pde ; 
    p = (pgtbl_entry_t *)(table_ptr) + PGTBL_INDEX(vaddr);

	// Check if p is valid or not, on swap or not, and handle appropriately

    if(!(p->frame & PG_VALID)){
 		//just allocate the frame and coremap and increase ref_count
    	miss_count++;
